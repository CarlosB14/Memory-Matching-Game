...
class Game{
    ...
    createBoxes() {
        this.#boxes = [];
        // Si hay datos en localStorage, creo las boxes desde ahí, si no nuevas
        if (localStorage.getItem('boxes') !== null){
            ...
            boxesFromLocalStorage.map(box => {
                let newBox = new Box(box.row, box.cl, box.color, box.free, box.open);
                this.#boxes.push(newBox);
            })

        } else {        
            let randomColors = this.createRandomColors();
            let arrayBoxesToLocalStorage = [];
    ...
    paintBoxes() {
        ...
        header.setAttribute('id', 'boxHeaderContainer');
    ...
    initTimer() {
        ...
        let header = document.getElementById('boxHeaderContainer');
